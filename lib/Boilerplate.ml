...
let map_float_lit (env : env) (tok : CST.float_lit) =
  (* float_lit *) token env tok

let map_decimal_lit (env : env) (tok : CST.decimal_lit) =
  (* decimal_lit *) token env tok

let map_octal_lit (env : env) (tok : CST.octal_lit) =
  (* octal_lit *) token env tok

let map_imm_tok_prec_p1_pat_dc28280 (env : env) (tok : CST.imm_tok_prec_p1_pat_dc28280) =
...
let map_key_type (env : env) (x : CST.key_type) =
  ...
  | `Sint32 tok -> R.Case ("Sint32",
      (* "sint32" *) token env tok
    )
  | `Sint64 tok -> R.Case ("Sint64",
      (* "sint64" *) token env tok
    )
  | `Fixed32 tok -> R.Case ("Fixed32",
      (* "fixed32" *) token env tok
    )
  | `Fixed64 tok -> R.Case ("Fixed64",
      (* "fixed64" *) token env tok
    )
  | `Sfixed32 tok -> R.Case ("Sfixed32",
      (* "sfixed32" *) token env tok
    )
  | `Sfixed64 tok -> R.Case ("Sfixed64",
      (* "sfixed64" *) token env tok
    )
...
let map_imm_tok_prec_p1_pat_3a2a380 (env : env) (tok : CST.imm_tok_prec_p1_pat_3a2a380) =
  (* pattern "[^\"\\\\]+" *) token env tok
...
let map_hex_lit (env : env) (tok : CST.hex_lit) =
  (* hex_lit *) token env tok

let map_string_ (env : env) (x : CST.string_) =
  (match x with
  | `DQUOT_rep_choice_imm_tok_prec_p1_pat_3a2a380_DQUOT (v1, v2, v3) -> R.Case ("DQUOT_rep_choice_imm_tok_prec_p1_pat_3a2a380_DQUOT",
      let v1 = (* "\"" *) token env v1 in
      let v2 =
        R.List (List.map (fun x ->
          (match x with
          | `Imm_tok_prec_p1_pat_3a2a380 x -> R.Case ("Imm_tok_prec_p1_pat_3a2a380",
  ...
  | `SQUOT_rep_choice_imm_tok_prec_p1_pat_dc28280_SQUOT (v1, v2, v3) -> R.Case ("SQUOT_rep_choice_imm_tok_prec_p1_pat_dc28280_SQUOT",
      let v1 = (* "'" *) token env v1 in
      let v2 =
        R.List (List.map (fun x ->
          (match x with
          | `Imm_tok_prec_p1_pat_dc28280 x -> R.Case ("Imm_tok_prec_p1_pat_dc28280",
...
let map_message_or_enum_type (env : env) ((v1, v2, v3) : CST.message_or_enum_type) =
  let v1 =
    (match v1 with
...
let map_field_number (env : env) (x : CST.field_number) =
  map_int_lit env x
...
let map_type_ (env : env) (x : CST.type_) =
  ...
  | `Mess_or_enum_type x -> R.Case ("Mess_or_enum_type",
      map_message_or_enum_type env x
    )
  )
...
let map_option_name (env : env) ((v1, v2) : CST.option_name) =
  let v1 =
    ...
    | `LPAR_full_id_RPAR (v1, v2, v3) -> R.Case ("LPAR_full_id_RPAR",
        let v1 = (* "(" *) token env v1 in
        let v2 = map_full_ident env v2 in
        let v3 = (* ")" *) token env v3 in
        R.Tuple [v1; v2; v3]
...
let rec map_constant (env : env) (x : CST.constant) =
  (match x with
  | `Full_id x -> R.Case ("Full_id",
      map_full_ident env x
    )
  | `Opt_choice_DASH_int_lit (v1, v2) -> R.Case ("Opt_choice_DASH_int_lit",
      let v1 =
        (match v1 with
        | Some x -> R.Option (Some (
  ...
  | `Opt_choice_DASH_float_lit (v1, v2) -> R.Case ("Opt_choice_DASH_float_lit",
      let v1 =
        (match v1 with
        | Some x -> R.Option (Some (
            map_anon_choice_DASH_81d4819 env x
  ...
  | `Blk_lit (v1, v2, v3) -> R.Case ("Blk_lit",
      let v1 = (* "{" *) token env v1 in
      let v2 =
        R.List (List.map (fun (v1, v2, v3, v4) ->
          ...
          let v3 =
            ...
            | `LBRACK_cst_rep_COMMA_cst_RBRACK (v1, v2, v3, v4) -> R.Case ("LBRACK_cst_rep_COMMA_cst_RBRACK",
                let v1 = (* "[" *) token env v1 in
...
let map_rpc (env : env) ((v1, v2, v3, v4, v5, v6, v7, v8, v9, v10, v11, v12) : CST.rpc) =
  ...
  let v12 =
    (match v12 with
    | `LCURL_rep_choice_opt_RCURL (v1, v2, v3) -> R.Case ("LCURL_rep_choice_opt_RCURL",
        let v1 = (* "{" *) token env v1 in
        let v2 =
          R.List (List.map (fun x ->
            (match x with
...
let map_map_field (env : env) ((v1, v2, v3, v4, v5, v6, v7, v8, v9, v10, v11) : CST.map_field) =
...
let map_enum_body (env : env) ((v1, v2, v3) : CST.enum_body) =
  ...
  let v2 =
    R.List (List.map (fun x ->
      ...
      | `Enum_field x -> R.Case ("Enum_field",
          map_enum_field env x
        )
      | `Empty_stmt tok -> R.Case ("Empty_stmt",
          (* ";" *) token env tok
        )
...
let map_oneof (env : env) ((v1, v2, v3, v4, v5) : CST.oneof) =
  ...
  let v4 =
    R.List (List.map (fun x ->
      ...
      | `Oneof_field x -> R.Case ("Oneof_field",
          map_oneof_field env x
        )
      | `Empty_stmt tok -> R.Case ("Empty_stmt",
          (* ";" *) token env tok
        )
...
and map_message_body (env : env) ((v1, v2, v3) : CST.message_body) =
  ...
  let v2 =
    R.List (List.map (fun x ->
      ...
      | `Oneof x -> R.Case ("Oneof",
          map_oneof env x
        )
      | `Map_field x -> R.Case ("Map_field",
          map_map_field env x
        )
      | `Rese x -> R.Case ("Rese",
          map_reserved env x
